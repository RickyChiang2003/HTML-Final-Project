...
for file in files:
    ...
    # 將資料分為訓練集和驗證集
    (X_train, X_test, Y_train, Y_test) = train_test_split(data2, labels, test_size=0.1, random_state=1126)
    ...
    if train == 1:
        # 訓練神經網路
        history = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), batch_size=64, epochs=50, verbose=1)
